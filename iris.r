...
# write to pdf
pdf(file="./pdfs/lab-plots.pdf",width=10,height=5)
...
p4 <- ggplot(data=iris, aes(x=Sepal.Width,colour=Species)) + geom_histogram() + theme(plot.title = element_text(size=5,hjust=TRUE)) + labs(x="Sepal Width",y="Sepal Length",title = "Box plot that visualizes the median and variance of Sepal Lengths across different species") + theme(plot.title = element_text(size=5))
